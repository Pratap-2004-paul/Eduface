// camera_mark_attendance.js - Mark attendance using facial recognition for teachers
...
async function captureAndRecognize() {
  ...
  try {
    const res = await fetch("/api/teacher/recognize_face", { method: "POST", body: fd });
...
if (trainModelBtn) {
  trainModelBtn.addEventListener("click", async () => {
    ...
    const start = await fetch("/api/teacher/train_model", { method: "POST" });
    ...
    const t = setInterval(async () => {
      const res = await fetch("/api/teacher/train_status");
      const data = await res.json();
      if (trainProgress) trainProgress.style.width = data.progress + "%";
      if (trainProgress) trainProgress.innerText = data.progress + "%";
      if (trainMsg) trainMsg.innerText = data.message || "";
